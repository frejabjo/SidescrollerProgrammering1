...
public class PhysicsCharacterController : MonoBehaviour
{
    ...
    //Jump 
    public float JumpSpeedFactor = 2.0f; //How much faster is the jump than the movespeed?
    ...
    private void Update()
    {
        ...
        // copy out HP-1 to new variable
        int hpCopy = HP - 1; 
        //if hpcopy is lesser(<) than zero(0), set it ro Zero(0)
        if (hpCopy < 0)
        {
            hpCopy = 0;
        }
    ...
    {
        Debug.Log("Should be taking damage"); //Josef
        HP += aHPValue;
    }
